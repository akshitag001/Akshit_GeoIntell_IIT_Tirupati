...
def add_spectral_indices(img_hwc: np.ndarray) -> np.ndarray:
    """
    Build 5-channel input: RGB + NDVI proxy + NDWI proxy.
    Input img_hwc expected in [0,1] float32 with shape (H,W,3).
    """
    red = img_hwc[:, :, 0].astype(np.float32)
    green = img_hwc[:, :, 1].astype(np.float32)
    blue = img_hwc[:, :, 2].astype(np.float32)
    ...
    return np.concatenate([img_hwc, ndvi, ndwi], axis=-1)
...
def load_checkpoint(checkpoint_path: Path, device: str):
    ...
    return cleaned_state, mean, std, class_names, num_classes
    ...
    with rasterio.open(tif_path) as src:
        ...
        for row in range(0, height, tile_size):
            for col in range(0, width, tile_size):
                ...
                tile_rgb = src.read([1, 2, 3], window=window).astype(np.float32) / 255.0
                ...
                # Build spectral channels from raw RGB, then normalize only RGB.
                # This matches train4.1_claude.py preprocessing.
                tile_rgb_norm = (tile_hwc - mean[:3]) / (std[:3] + 1e-6)
                extra_channels = add_spectral_indices(tile_hwc)[:, :, 3:]
                tile_5ch = np.concatenate([tile_rgb_norm, extra_channels], axis=-1)

                tile_chw = np.transpose(tile_5ch, (2, 0, 1))
...
def main():
    parser = argparse.ArgumentParser(
        description="Run DeepLab full-image segmentation on a GeoTIFF and export class-wise shapefiles."
    )
    parser.add_argument("--input-tif", required=True, help="Path to input .tif")
    parser.add_argument("--checkpoint", default="deeplab_best_v4.pth", help="DeepLab checkpoint path")
    parser.add_argument("--output-dir", default="predicted_shp_deeplab", help="Output folder for shapefiles")
    parser.add_argument("--num-classes", type=int, default=5, help="Number of classes in model")
    ...
    parser.add_argument(
        ...
        help="Class mapping format: '1:Built_Up_Area_typ,2:Road,3:Water_Body,4:Farm'",
    )
    ...
    state_dict, mean, std, ckpt_class_names, ckpt_num_classes = load_checkpoint(checkpoint_path, device)

    num_classes = ckpt_num_classes if ckpt_num_classes is not None else args.num_classes
    model = build_model(num_classes=num_classes, state_dict=state_dict, device=device)
    ...
    print(f"Running tiled DeepLab inference on: {input_tif}")
    ...
    print("Done. DeepLab shapefiles are ready.")
